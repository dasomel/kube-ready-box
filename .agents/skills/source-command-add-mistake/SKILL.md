...
Use this skill when the user asks to run the migrated source command `add-mistake`.
...
이 명령어를 실행하면 실수 패턴을 `.Codex/cache/mistake-candidates.jsonl`에 기록합니다.
...
```bash
cat .Codex/cache/mistake-candidates.jsonl | jq .
```
