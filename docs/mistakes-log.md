...
> 단일 출처(SSOT). `AGENTS.md`와 `CLAUDE.md`는 이 파일을 링크만 하고 내용을 복제하지 않습니다 —
> 실제로 두 파일에 각자 사본을 두었다가 12번 항목이 어긋난 적이 있습니다(레거시 `dhcp-identifier: mac`
> 고정 방식이 `AGENTS.md`에만 새 해결책으로 갱신되고 `CLAUDE.md`엔 옛 내용이 남아있었음).
> 새 실수를 발견하면 여기에만 추가하세요 (`/add-mistake` 참고).
