...
| `security` | `validate.yml`: `supply-chain-guard` (#30 unpinned-input static check), `supply-chain-negative-tests` (live egress-block + checksum-substitution tests); all four Ubuntu Packer builds run `tools/image-identity-security-check.sh` after final cleanup, retain `identity-security-report.json`, and remove the uploaded check inputs before packaging; host firewall/MAC/seccomp baseline inventory (#44, see [docs/host-security-baseline.md](host-security-baseline.md)) runs as the `network`/`security` aggregator reports on every PR | PR checks automated; image check runs only during a real box build |
