...
# 메모리 맵 영역 최대값 (mmap 다용 워크로드: Elasticsearch 등 / K8s 노드)
# 값 1048576: Ubuntu 26.04 systemd 기본(/usr/lib/sysctl.d/55-map-count.conf)과 동일.
# 99-* 파일이 55-* 보다 늦게 적용되므로, 낮은 값을 쓰면 26.04 기본을 다운그레이드함 → 기본값에 맞춤.
# 24.04(systemd 기본 없음, base 65536)에서는 상향 효과.
