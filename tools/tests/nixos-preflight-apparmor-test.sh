#!/usr/bin/env bash
# Deterministic fixture coverage for #44 T-021: nixos/preflight.sh's
...
SCRIPT="$ROOT/nixos/preflight.sh"
WORKDIR="$(mktemp -d)"
...
assert_check "$out" apparmor UNKNOWN facility-absent

echo "nixos-preflight-apparmor-test.sh: all scenarios passed"
